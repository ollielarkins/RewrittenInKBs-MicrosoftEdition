...
1. Open the live app in Chrome or Edge:
   https://ollielarkins.github.io/RewrittenInKBs-MicrosoftEdition/notepad/
...
```bash
git clone https://github.com/ollielarkins/RewrittenInKBs-MicrosoftEdition.git
cd RewrittenInKBs-MicrosoftEdition
python -m http.server 3000
```

Then open `http://localhost:3000/notepad/`. (Save-back-to-disk needs a
`localhost`/`https` origin — on the installed app and the live URL it works;
double-clicking the raw `index.html` falls back to a download.)
...
Part of the [Rewritten in KBs](../README.md) series repo. GitHub Pages serves
the repo from `main` (no build, no workflow — the files *are* the site;
`.nojekyll` at the repo root tells Pages to skip Jekyll), so this project lives
at `/notepad/`. See the root README for the one-time Pages setup.
